...
def main():
    ...
    baudrate = 115200
    bus_address = 5
    output_file = "tests/data_log.csv"
    ...
    with open(output_file, 'w', newline='') as csvfile:
        fieldnames = ['timestamp', 'pressure_torr', 'current_mA', 'voltage_V']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()

        print(f"\nLogging data to {output_file}...")
        print("Press Ctrl+C to stop\n")
        print(f"{'#':<6} {'Timestamp':<26} {'Pressure (Torr)':<18} {'Current (mA)':<15} {'Voltage (V)':<12}")
        print("-" * 80)
